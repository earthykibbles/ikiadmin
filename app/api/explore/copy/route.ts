...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!sourceDateId || !targetDateIds || !Array.isArray(targetDateIds) || targetDateIds.length === 0) {
    ...
    if (!sourceDoc.exists) {
      return NextResponse.json(
        { error: `Source date ${sourceDateId} not found` },
        { status: 404 }
      );
    }
    ...
    if (sourceVideos.length === 0) {
      return NextResponse.json(
        { error: 'Source date has no videos to copy' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to copy explore page' },
      { status: 500 }
    );
  }
}
