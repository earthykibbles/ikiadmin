...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const task = (body?.task || request.nextUrl.searchParams.get('task') || 'all').toLowerCase();
    const limitRaw = body?.limit ?? request.nextUrl.searchParams.get('limit') ?? 100;
    const limit = Math.min(Math.max(parseInt(String(limitRaw), 10) || 100, 1), 500);

    const results: Record<string, unknown> = { task, limit };
    ...
    async function processBroadcasts(batchSize: number) {
      ...
      for (const b of broadcastsSnap.docs) {
        ...
        const senderName = String(bd.sender_name || 'IKI Admin').trim() || 'IKI Admin';
        const senderAvatar =
          String(bd.sender_avatar || '').trim() || dicebearAvatarUrl('iki_admin');
    ...
    const message = error instanceof Error ? error.message : 'Failed to run notification router';
